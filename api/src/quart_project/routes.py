...
def setup_routes(app, cosmos_repository, storage_container_repository):
    ...
    async def create_document(user: User):
        ...
        use_case = CreateDocument(user, DocumentRepository(cosmos_repository), StorageDocumentRepository(storage_container_repository), themeRepository=ThemeRepository(cosmos_repository))
        if files == None:
